...
test("should register sample user properly and use its info to authorize and provide a random token, then, logout with the token provided", async () => {
    ...
    //LOGIN
    let dataResponse = await authService.login(sampleUserInfo.username, sampleUserInfo.password);
    ...
    let hasNullValue = Object.values(dataResponse).some(value => value === null);
    ...
    expect(Object.prototype.toString.call(new Date(sessionData.sessionStart))).toBe('[object Date]');
    expect(Object.prototype.toString.call(new Date(sessionData.sessionEnd))).toBe('[object Date]');
